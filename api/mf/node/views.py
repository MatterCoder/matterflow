...
from matterflow import Workflow, WorkflowException, Node, NodeException, node_factory, ParameterValidationError
...
def node(request):
    ...
    if new_node.is_global:
        node_exists = request.matterflow.get_flow_var(new_node.node_id)
    else:
        node_exists = request.matterflow.get_node(new_node.node_id)
    ...
    # Add to the graph
    try:
        request.matterflow.update_or_add_node(new_node)
...
def handle_edge(request, node_from_id, node_to_id):
    ...
    node_from = request.matterflow.get_node(node_from_id)
    node_to = request.matterflow.get_node(node_to_id)
    ...
    try:
        if request.method == 'POST':
            response = JsonResponse({
                'edge_added': request.matterflow.add_edge(node_from, node_to)
            })
        elif request.method == 'DELETE':
            response = JsonResponse({
                'removed_edge': request.matterflow.remove_edge(node_from, node_to)
            })
        else:
            return JsonResponse({
...
def handle_node(request, node_id):
    ...
    if request.path_info.startswith('/node/global/'):
        retrieved_node = request.matterflow.get_flow_var(node_id)
    else:
        retrieved_node = request.matterflow.get_node(node_id)
    ...
    try:
        if request.method == 'GET':
            response = JsonResponse({
                ...
                "flow_variables": request.matterflow.get_all_flow_var_options(node_id),
            }, safe=False)
            ...
            request.matterflow.update_or_add_node(updated_node)
            response = JsonResponse(updated_node.to_json(), safe=False)
        elif request.method == 'DELETE':
            request.matterflow.remove_node(retrieved_node)
            response = JsonResponse({
...
def execute_node(request, node_id):
    ...
    try:
        ...
        executed_node = request.matterflow.execute(node_id)

        # Save executed Node, with new data, back to Workflow
        request.matterflow.update_or_add_node(executed_node)
...
def retrieve_data(request, node_id):
    try:
        node_to_retrieve = request.matterflow.get_node(node_id)
        data = request.matterflow.retrieve_node_data(node_to_retrieve)
