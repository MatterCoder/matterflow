import unittest
from matterflow import Workflow, WorkflowException, Node, NodeException, node_factory
...
class MatterflowTestCase(unittest.TestCase):
    ...
    def create_workflow(self):
        ...
        for node in self.nodes:
            self.matterflow.update_or_add_node(node)

        for edge in self.edges:
            source_node = self.matterflow.get_node(edge[0])
            target_node = self.matterflow.get_node(edge[1])
            self.matterflow.add_edge(source_node, target_node)

    def test_get_local_flow_nodes(self):
        node_with_flow = self.matterflow.get_node("3")
        flow_nodes = self.matterflow.load_flow_nodes(node_with_flow.option_replace)
        self.assertEqual(len(flow_nodes), 1)

    def test_get_global_flow_nodes(self):
        node_with_flow = self.matterflow.get_node("2")
        flow_nodes = self.matterflow.load_flow_nodes(node_with_flow.option_replace)
        self.assertEqual(len(flow_nodes), 1)

    def test_get_global_flow_node_exception(self):
        node_with_flow = self.matterflow.get_node("1")
        flow_nodes = self.matterflow.load_flow_nodes(node_with_flow.option_replace)
        self.assertEqual(len(flow_nodes), 0)

    def test_get_execution_order(self):
        self.create_workflow()
        order = self.matterflow.execution_order()
    ...
    def test_xexecute_workflow(self):
        ...
        for node in order:
            executed_node = self.matterflow.execute(node)
            self.matterflow.update_or_add_node(executed_node)

    # def test_execute_workflow_load_data(self):
    #     print(self.matterflow.graph.nodes)
    #     data = self.matterflow.load_input_data("3")
